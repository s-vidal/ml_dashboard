...
class DataHandler {
  ...
  getFeaturePermutations = (checkedFeatures) => {
    ...
    for (const [key, value] of Object.entries(permutations)) {
      ...
      const hasAllKeys = permutationKeys.every((item) =>
        permutation.hasOwnProperty(item)
      );
      if (permutationKeys.length === featuresLength && hasAllKeys) {
        filteredFeaturesArray.append(permutation);
      }
    }
